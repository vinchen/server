...
)
{
	...
	ulint               i = 0;
	dfield_t						dfield;
	byte*								null_byte = table->s->default_values;
#define DATA_INT_MAX_LEN 8
	...
	if (field->default_value && field->has_default_now_unireg_check()) {
		...
		//ut_ad(item_def->type() == Item::FUNC_ITEM && 
		//	((Item_func*)item_def)->functype() == Item_func::NOW_FUNC );
		THD* thd = current_thd;

		// see Item::save_date_in_field
		MYSQL_TIME ltime;
		//if (item_def->get_date(&ltime, 0 /*sql_mode_for_dates(current_thd)*/)) {
