...
{
  sp_name tmp(thd->db_lex_string(), package, false);
  if (!sp_check_if_package_exists(thd, &tmp))
    return false;
  return name->make_package_routine_name(thd, thd->db_lex_string(),
                                         package, routine);
}
                                        ...
                                        const LEX_STRING &routine)
{
  ...
  sp_name tmp2(thd->db_lex_string(), package, false);
  if (!sp_check_if_package_exists(thd, &tmp2))
    return false;

  if (tmp.make_package_routine_name(thd, thd->db_lex_string(), package, routine))
    return true;
